...
def main():
    ...
    ## IBOVESPA
    ibov_df = web.DataReader('^BVSP', data_source='yahoo', start = '2020-01-02', end = f'{data_atual}') #web.DataReader('TICKER', data_source='fonte', start = 'data inicio', end = 'data final')
    ...
    for ativo in carteira['Ativos']:
        if 'Tesouro' not in ativo: #retirando o tesouro direto do FOR (Não da para retirar as informações dele pelo link do yahoo)
            carteira_df[ativo] = web.DataReader('{}.SA'.format(ativo), data_source='yahoo', start = '2020-01-01', end = f'{data_atual}')['Adj Close'] #pegando os dados dos ativos da carteira
        else:
            pass
    ...
    plt.legend()
    fig4 = grafico4.figure #atribuindo gráfico a essa variavel
    fig4.savefig(COMPARATIVO_CARTEIRA_IBOV) #salvando gráfico no caminho especificado

    print("Iniciando rotina de envio de email")
